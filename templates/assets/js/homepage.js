...
function initialize() {
    ...
    if(location.search.length > 1){
        ...
        query[queries["type"]] = queries["item"];
        $("#adv_search").click();
    }
...
function loadGeoJson(query, resize) {
    ...
    function _callback(data) {
        resize || (resize = false);
        var total_count = data[0].O.count;
        loadGeoJsonCallback(resize, total_count);
    }

    bigMap.data.loadGeoJson('/api/v1/location/search/?' + $.param(query), null, _callback);
}

function loadGeoJsonCallback(resize, total_count) {
    //set the extend
    ...
    case 1:
        ...
        break;
    default:
        count = count + ' of ' + total_count +' results';
    }
...
function DirectoryRecord(feature, parentNode) {
    ...
    this.setContentString = function(str) {
        var max_words = 50;
        var desc = this.feature.getProperty('description').split(" ");
        if (desc.length > max_words) {
            desc = desc.splice(0, max_words).join(" ") + " ... ";
        } else {
            desc = desc.join(" ");
        }
...
$('#btn-map-tab').click(function () {
    //google maps is not showing correctly when the map is invisible.
    //So adjust the bound again when click on the show map button.
    //And google maps too fast. So make it slow a bit.
    setTimeout(function() {
...
$('#btn-directory-tab').click(function () {
    //google maps is not showing correctly when the map is invisible.
    //So adjust the bound again when click on the show map button.
    //And google maps too fast. So make it slow a bit.
    setTimeout(function() {
        DirectoryManager.adjustMap();
    }, 100);
});
