/**
 * Created by SULEIMAN on 1/13/15.
 */

(function () {
    mapp_app.provider('formTracker', function () {
        var trackers = {};
        ...
        this.$get = ['$q', '$timeout', function ($q, $timeout) {
            ...
            return function formTracker(options) {
                //do new if user doesn't
                if (!(this instanceof formTracker)) {
                ...
                self.addForm = function (promise) {
                    var then = promise && (promise.then || promise.$then ||
                        (promise.$promise && promise.$promise.then));
                    if (!then) {
                        throw new Error("formTracker#addPromise expects a promise object!");
                    }
