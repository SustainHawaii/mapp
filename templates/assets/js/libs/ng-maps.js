  ...
  .directive('geojson', ['$http', function($http) {
    return {
      ...
      scope: {
        ...
        events: '=',      // object {event:function(), event:function()}
        visible: '=',     // boolean
        pointoptions: '=',     // function() { return {} }
        polygonoptions: '=',     // function() { return {} }
        opacity: '=',     // int <= 100 //used for polygons
        data: '=?',    //optional binding
      },
      require: '^map',
      link: function($scope, $element, $attrs, parent) {
        ...
        $scope.$watch(function() {
          ...
          $scope.$watch("pointoptions", function() {
            angular.forEach(markers, function(m, i) {
              marker.setOptions($scope.pointoptions(m.geometry, m.properties, map, i));
            });
          });


          $scope.$watch('polygonoptions', function() {
            angular.forEach(polygons, function(p, i) {
              var opts = $scope.polygonoptions ? $scope.polygon_options(p.geometry, p.properties, map, i) : {};
      ...
      function PolygonCollection(p, i) {
        ...
        var opts = $scope.polygonoptions ? $scope.polygonoptions(p.geometry, p.properties, i, map) : {};
        opts.fillOpacity = $scope.opacity ? $scope.opacity/100 : 1;
      ...
      var getMultiPolyBounds = function(geo) {
        /* this takes in a polygoncollection */
          var bounds = new google.maps.LatLngBounds()
          geo.polygons.forEach(function(element, index){
          ...
          var newData = function(url) {
            ...
            $http.get(url).success(function(data) {
              ...
              //keep track of where all the markers are so we can adjust map
              mapBound = new google.maps.LatLngBounds();
              hasFeatures = false;

              angular.forEach(data.features, function(m, i) {

                if ((m.geometry.type == "MultiPolygon") || (m.geometry.type == "Polygon")) {
                  var PC = new PolygonCollection(m, i);

                  hasFatures = true;
                  
                  polygons.push(PC);
                  
                  poly_bounds = getMultiPolyBounds(PC);
                  mapBound.extend(poly_bounds.getNorthEast());
                  mapBound.extend(poly_bounds.getSouthWest()); 
                ...
                var opts = $scope.pointoptions ? $scope.pointoptions(m.geometry, m.properties, map, i) : {};
                ...
                //update new map geometry
                mapBound.extend(opts.position);
...
.directive('map', [function() {
  ...
  link: function($scope, elem, attrs) {
    ...
    //when refresh set proper bounds
    function setBounds() {
    //set the extend
    mapBound = new google.maps.LatLngBounds();
